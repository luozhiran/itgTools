...
│   ├── bind(tabLayout, viewPager, tabs)    │   └── 同上
│   ├── selectTab(pos)                      │
...
override fun onCreateTabConfig() = TabConfig(
    ...
    style = ...,                          // 视觉样式（见下方）
    onPageSelected = { pos, fragment -> },// 页面切换回调
)
```
...
TabItemStyle(
    ...
    iconGravity = TabLayout.ICON_GRAVITY_TOP,  // TOP / START / END / BOTTOM
    iconSizeDp = 24,                            // 图标大小
    minWidthDp = 80,                            // Tab 最小宽度
    selectedBackground = R.drawable.bg_tab_active,   // 选中背景
...
override fun onCreateTabs() = listOf(
    TabItem("首页", R.drawable.ic_home, fragmentClass = HomeFragment::class.java),
    TabItem("发现", R.drawable.ic_discover, fragmentClass = DiscoverFragment::class.java),
    TabItem("消息", R.drawable.ic_msg, fragmentClass = MessageFragment::class.java,
            badge = TabBadge(count = 99)),
    TabItem("我的", R.drawable.ic_me, fragmentClass = ProfileFragment::class.java),
)
```
...
| LifeAbility 自动解绑 | `onDestroy` 时 detach TabLayoutMediator、移除 PageChangeCallback |
| Event 一次性消费 | Tab 切换事件配置变更后不重复触发 |
| viewLifecycleOwner | Fragment 视图重建时自动解绑 LiveData 观察者 |
| AutoClearedValue | Adapter 和回调引用在 `onDestroyView` 时显式清理 |
...
| **L3 整体替换** | 子类化 `TabViewPagerAbility` | 完全不同的 Tab 行为 | `class MyTabAbility : TabViewPagerAbility()` |
