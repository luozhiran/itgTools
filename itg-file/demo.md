...
> **文档导航：** [← 返回 README](./README.md) ｜ [清理指南 →](./FILE_CLEANUP_GUIDE.md)
...
- [两种模式对比](#两种模式对比)
- [1. 文件基础操作](#1-文件基础操作)
  - [1.1 检查文件是否存在](#11-检查文件是否存在)
  - [1.2 创建文件/目录](#12-创建文件目录)
  - [1.3 删除文件/目录](#13-删除文件目录)
  - [1.4 重命名](#14-重命名)
  - [1.5 复制文件/目录](#15-复制文件目录)
  - [1.6 移动文件/目录](#16-移动文件目录)
  - [1.7 列出文件](#17-列出文件)
  - [1.8 获取文件信息](#18-获取文件信息)
  - [1.9 存储空间查询](#19-存储空间查询)
- [2. 文件读取](#2-文件读取)
  - [2.1 读取为字符串](#21-读取为字符串)
  - [2.2 读取为字节数组](#22-读取为字节数组)
  - [2.3 按行读取](#23-按行读取)
  - [2.4 流式逐行读取（大文件友好）](#24-流式逐行读取大文件友好)
  - [2.5 分块读取（超大二进制文件）](#25-分块读取超大二进制文件)
  - [2.6 读取文件头部/尾部](#26-读取文件头部尾部)
  - [2.7 从 URI / InputStream 读取](#27-从-uri--inputstream-读取)
  - [2.8 超时读取（Okio 独有）](#28-超时读取okio-独有)
  - [2.9 带进度读取（Okio 推荐）](#29-带进度读取okio-推荐)
  - [2.10 Gzip 解压读取（Okio 独有）](#210-gzip-解压读取okio-独有)
- [3. 文件写入](#3-文件写入)
  - [3.1 写入字符串](#31-写入字符串)
  - [3.2 追加字符串](#32-追加字符串)
  - [3.3 写入字节数组](#33-写入字节数组)
  - [3.4 从 InputStream 写入](#34-从-inputstream-写入)
  - [3.5 写入到 Content URI](#35-写入到-content-uri)
  - [3.6 原子写入详解](#36-原子写入详解)
  - [3.7 分块写入大文件（带进度）](#37-分块写入大文件带进度)
  - [3.8 超时写入（Okio 独有）](#38-超时写入okio-独有)
  - [3.9 Gzip 压缩写入（Okio 独有）](#39-gzip-压缩写入okio-独有)
- [4. 文件哈希与校验](#4-文件哈希与校验)
  - [4.1 计算文件哈希](#41-计算文件哈希)
  - [4.2 大文件哈希（带进度回调）](#42-大文件哈希带进度回调)
  - [4.3 校验文件完整性](#43-校验文件完整性)
  - [4.4 比较两个文件](#44-比较两个文件)
  - [4.5 字节/字符串哈希](#45-字节字符串哈希)
  - [4.6 复制并同时计算哈希（Okio 独有）](#46-复制并同时计算哈希okio-独有)
  - [4.7 复制并验证（Okio 独有）](#47-复制并验证okio-独有)
  - [4.8 Gzip 压缩并计算哈希（Okio 独有）](#48-gzip-压缩并计算哈希okio-独有)
  - [4.9 写入时哈希（Okio 独有）](#49-写入时哈希okio-独有)
- [5. Assets / Raw 资源读取](#5-assets--raw-资源读取)
  - [5.1 列出与检查 Assets 资源](#51-列出与检查-assets-资源)
  - [5.2 读取 Assets 文件内容](#52-读取-assets-文件内容)
  - [5.3 复制 Assets 到文件系统](#53-复制-assets-到文件系统)
  - [5.4 读取 Raw 资源](#54-读取-raw-资源)
  - [5.5 复制 Raw 资源到文件系统](#55-复制-raw-资源到文件系统)
  - [5.6 Assets 超时读取与 Gzip 解压（Okio 独有）](#56-assets-超时读取与-gzip-解压okio-独有)
- [6. 文件清理（生命周期驱动）](#6-文件清理生命周期驱动)
  - [6.1 应用启动时清理](#61-应用启动时清理)
  - [6.2 应用进入后台时清理](#62-应用进入后台时清理)
  - [6.3 延迟清理](#63-延迟清理)
  - [6.4 按天数清理（跨重启持久化）](#64-按天数清理跨重启持久化)
  - [6.5 指定时间点清理](#65-指定时间点清理)
  - [6.6 立即执行（runNow）](#66-立即执行runnow)
  - [6.7 权限处理](#67-权限处理)
  - [6.8 生命周期绑定详解](#68-生命周期绑定详解)
  - [6.9 取消与释放](#69-取消与释放)
- [模式选择速查表](#模式选择速查表)
- [同步 vs 异步](#同步-vs-异步)
